...
--câu e--
select* from nhanvien where macv='CV';
--câu f--
select * from nhanvien where tennv like 't%';
--câu g--
select * from nhanvien where month (nhanvien.ngaysinh)=8;
--câu h hsluong duoi 3.00--
select*from nhanvien,quatrinhluong where nhanvien.manv=quatrinhluong.manv and quatrinhluong.hsluong<3.00;
--câu i nhanvien không còn công tác--
...
group by nhanvien.holot +' '+nhanvien.tennv, sau.hsluong, sau.ngaybd

---cau k----tinh lương gồm các cột : họ tên nhan viên, giơi tính , tiền lương, phụ cấp, thực lĩnh
...
select chucvu.tencv, COUNT(manv) as tongnhanvien from nhanvien, chucvu
where nhanvien.macv = chucvu.macv
group by chucvu.macv, chucvu.tencv


---cau m---Cho biết các nhân viên có tuổi nhỏ hơn độ tuổi trung bình, gồm: Họ tên, Giới tính, Tuổi.
select nhanvien.holot+' '+nhanvien.tennv as hoten, nhanvien.phai,
DATEDIFF(YEAR, nhanvien.ngaysinh,GETDATE()) as  Tuoi from nhanvien
where DATEDIFF(YEAR, nhanvien.ngaysinh,GETDATE()) < (select AVG(DATEDIFF(year, nhanvien.ngaysinh,GETDATE())) from nhanvien)

---cau n---Cho biết tổng số nhân viên nam , tổng số nhân viên nữ đang công tác.
